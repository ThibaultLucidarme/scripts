...
apt-get install -y acpi-support alsa-base apparmor apparmor-profiles apparmor-utils avahi-autoipd avahi-daemon avahi-utils grub-pc linux-image-generic network-manager pm-utils ubuntu-minimal ubuntu-standard x11-xserver-utils xserver-xorg xserver-xorg-core xserver-xorg-input-evdev xserver-xorg-video-fbdev xserver-xorg-video-modesetting xserver-xorg-video-nouveau xserver-xorg-video-vesa


# Install Unity

apt-get install -y activity-log-manager dmz-cursor-theme language-selector-gnome lightdm notify-osd overlay-scrollbar plymouth-theme-ubuntu-logo sni-qt ubuntu-artwork ubuntu-docs ubuntu-minimal ubuntu-session ubuntu-settings ubuntu-standard ubuntu-wallpapers unity unity-control-center unity-settings-daemon zeitgeist
...
apt-get install -y apt-xapian-index apturl gdebi-core synaptic update-manager update-notifier
...
apt-get install -y chromium-browser firefox gajim thunderbird thunderbird-gnome-support xul-ext-lightning


# Install multimedia

apt-get install -y brasero ffmpegthumbnailer vlc
...
apt-get install -y calibre gimp
...
apt-get install -y exfat-fuse exfat-utils ntfs-3g unrar
